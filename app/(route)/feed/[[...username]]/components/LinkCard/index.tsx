...
const LinkCard = ({ link, isOwner }: LinkCardProps) => {
  ...
  return (
    <Link href={url ?? ""} target="_blank" className="relative">
        ...
        {title && (
          <div className=" bg-black w-2/3 p-4 flex flex-col justify-between">
            <div>
              ...
              <p className="text-neutral-400 text-xs line-clamp-3 mt-1">
                {description}
              </p>
            </div>
      ...
      {isOwner && (
        <Input
          className="text-neutral-400 mt-1 placeholder:text-neutral-600 bg-transparent border-none focus:border-none focus-visible:ring-offset-0 focus-visible:ring-transparent"
      ...
      {!isOwner && (
        <p
          className="text-neutral-400 mt-1 cursor-default"
          onClick={(e) => e.preventDefault()}
        >
          {caption}
        </p>
